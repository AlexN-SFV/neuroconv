...
## Features
* `EDFRecordingInterface` now supports discontinuous EDF+ (`EDF+D`) files, which were previously rejected outright and which neither SpikeInterface's reader nor `pyedflib` beneath it can open. `EDF+D` is detected from the header and read with a pure-Python/NumPy reader that recovers each data record's true start time from the time-keeping annotations in the `EDF Annotations` signal. Each contiguous run of records becomes its own `ElectricalSeries` at its real start time, sharing one electrodes table, with `runs` and `annotations` `TimeIntervals` tables alongside (disable with `write_runs=False` / `write_annotations=False`). Samples are read lazily, so memory does not scale with file size. The reader tolerates writers that never NUL-terminate a TAL and only NUL-pad the tail of the annotations block, as Nihon Kohden's export does, and where a record's time-keeping annotation is genuinely unreadable it derives the start time from the neighbouring records whenever they span exactly the records in between, warning rather than refusing the file. Plain EDF and `EDF+C` files are unaffected and still read via SpikeInterface. See the [gallery page](https://neuroconv.readthedocs.io/en/main/conversion_examples_gallery/recording/edf.html) for details.
