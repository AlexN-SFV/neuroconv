...
* Deprecated the public `add_devices_to_nwbfile` (ecephys). The function now emits a `FutureWarning` and only supports the old list-based `metadata["Ecephys"]["Device"]` format. New code should pass dict-based metadata (`metadata["Devices"]`) to `add_recording_to_nwbfile`, or call `_add_device_to_nwbfile` from `tools.nwb_helpers` directly for a single device. Will be removed on or after November 2026. [PR #1743](https://github.com/catalystneuro/neuroconv/pull/1743)
* Stopped calling the removed `get_accepted_list` / `get_rejected_list` methods on segmentation extractors. ROI acceptance is now written automatically as a column on the `PlaneSegmentation` table from the extractor's property system (e.g. `is_accepted`, `is_rejected`). The `include_roi_acceptance` parameter on `add_segmentation_to_nwbfile`, `write_segmentation_to_nwbfile`, `BaseSegmentationExtractorInterface.add_to_nwbfile`, `Suite2pSegmentationInterface.add_to_nwbfile`, and `MinianSegmentationInterface.add_to_nwbfile` is now a deprecated no-op and will be removed on or after November 2026. [PR #1736](https://github.com/catalystneuro/neuroconv/pull/1736)
* Bumped minimum `spikeinterface` to `>=0.104.7` and `neo` to `>=0.14.5`, removing the previous `neo<0.14.5` cap now that released SpikeInterface no longer passes `load_sync_channel` to neo's `SpikeGLXRawIO`. [PR #1766](https://github.com/catalystneuro/neuroconv/pull/1766)
* Updated the URL for EMBER with `sandbox=True` for the `automatic_dandi_upload` helper function. Also updated the testing suite to use a new sandbox Dandiset.

## Bug Fixes
* Fixed EDF files carrying a logarithmically transformed signal converting silently wrong. Such a signal is marked by the physical dimension `Filtered` and stores logarithms rather than measurements ([specification](https://www.edfplus.info/specs/edffloat.html)); SpikeInterface's reader has no notion of it and applies the header's linear gain and offset, so a signal spanning 0.05 to 5e6 µV was read back as roughly 24 to 306 µV with no error and no warning. `EDFRecordingInterface` now detects such signals from the header and decodes them to physical values on top of the usual reader. The recording's dtype becomes `float64`, and a signal whose parameters are unreadable is refused rather than guessed at — drop it with `channels_to_skip`. Files with no transformed signal are unaffected.
...
* Fixed every test job failing on pull requests opened from a fork. GitHub withholds repository secrets from those runs, so the `load-data` action's AWS credentials arrived empty and the first `aws s3 ls` aborted the job while computing a cache key, before a single test ran. The action now detects the missing credentials, skips the listing, and restores the most recent cached copy of the test data through `restore-keys`. This also lets the live service job reach its own credential check, whose message explaining that those tests need maintainer secrets was previously unreachable behind the data step. Runs that do have credentials are unaffected. [PR #1836](https://github.com/catalystneuro/neuroconv/pull/1836)
...
* Fixed the daily formatwise gallery job failing on every OS and Python combination. The job derives an installation extra from each gallery page's filename, but `multifile_csv_fp.rst` had no matching extra, so the environment was built without `ndx-fiber-photometry` and the doctest died on import. `MultiFileCSVFiberPhotometryInterface` is a layout variant of the same CSV format rather than a separate format, and the page already told readers to install `neuroconv[csv_fp]`, so it is now a section of the CSV fiber photometry gallery page instead of a page of its own. [PR #1827](https://github.com/catalystneuro/neuroconv/pull/1827)
* Prevented different `Devices` and `DeviceModels` metadata keys from using the same NWB name. Duplicate names now raise an error before the NWB object is added. [PR #1824](https://github.com/catalystneuro/neuroconv/pull/1824)
* Fixed fiber photometry interfaces requiring two fields the source never provides and raising `KeyError` before writing. The optical fiber's device model is now optional (a model is attached only when `device_model_metadata_key` is supplied, matching ndx-ophys-devices where `model` is optional), and the response series' `DynamicTableRegion` description now defaults at construction (as ecephys does with its hardcoded `"electrode_table_region"`) instead of requiring `fiber_photometry_table_region_description`. [PR #1807](https://github.com/catalystneuro/neuroconv/pull/1807)
* Fixed `EDFRecordingInterface.get_metadata` discarding every subject field it read from the EDF header. `metadata` is a `DeepDict`, so `metadata.get("Subject", dict()).update(...)` updated a throwaway dict and neither `subject_id` nor `date_of_birth` ever reached the NWB file. The subject's sex is now read as well, and the birthdate is parsed into the `datetime` that `Subject.date_of_birth` requires. A file whose header carries any subject information therefore gains a `Subject` where previously it had none. [PR #1841](https://github.com/catalystneuro/neuroconv/pull/1841)
...
## Features
* `EDFRecordingInterface` now supports discontinuous EDF+ (`EDF+D`) files, which were previously rejected outright and which neither SpikeInterface's reader nor `pyedflib` beneath it can open. `EDF+D` is detected from the header and read with a pure-Python/NumPy reader that recovers each data record's true start time from the time-keeping annotations in the `EDF Annotations` signal. Each contiguous run of records becomes its own `ElectricalSeries` at its real start time, sharing one electrodes table, with `runs` and `annotations` `TimeIntervals` tables alongside (disable with `write_runs=False` / `write_annotations=False`). Samples are read lazily, so memory does not scale with file size. The reader tolerates writers that never NUL-terminate a TAL and only NUL-pad the tail of the annotations block, as Nihon Kohden's export does; that tolerance engages only for a file that does not parse under the spec alone and terminates no TAL anywhere, so a conformant file is read by the spec and nothing else. Where a record's time-keeping annotation is genuinely unreadable, the start time is derived from the neighbouring records whenever they span exactly the records in between, warning rather than refusing the file. The recording exposes `physical_unit`, `gain_to_physical_unit` and `offset_to_physical_unit` per channel, as the SpikeInterface path already did, so code that checks the header's declared unit against an external source such as a BIDS `channels.tsv` works on continuous and discontinuous files alike, and an auxiliary channel written as a `TimeSeries` carries its real unit instead of `n.a.`. Plain EDF and `EDF+C` files are unaffected and still read via SpikeInterface. See the [gallery page](https://neuroconv.readthedocs.io/en/main/conversion_examples_gallery/recording/edf.html) for details.
* Added `neuroconv.__version__`, exposing the installed version at the package root. SpikeInterface stamps the defining package's version into an extractor's provenance dict and reads `<package>.__version__` back when reloading it, so a neuroconv-owned extractor could not previously be pickled or reloaded.
* Added `BrukerTiffImagingInterface`, a unified interface for Bruker Prairie View OME-TIFF imaging data backed by the new unified `BrukerTiffImagingExtractor` from roiextractors. Single-plane, volumetric, and multi-channel cases are handled through one class, with channels selected by name via `channel_name` (e.g. `"Ch1"`, `"Ch2"`). The interface emits metadata in the new dict-based format only (`Devices`, `Ophys.ImagingPlanes`, `Ophys.MicroscopySeries`), and reads each imaging plane's focal depth from the active `zDevice` so `origin_coords`, `grid_spacing`, and `field_of_view` carry the correct microscope-frame geometry. [PR #1804](https://github.com/catalystneuro/neuroconv/pull/1804)
...
* Added `MockIcephysInterface`, a synthetic intracellular electrophysiology interface that writes a `PatchClampSeries` and one intracellular-recordings row per sweep, for testing the format-independent icephys machinery without acquisition files. The Axon converter's grouping-level and electrode-sharing tests now run through it rather than downloading ABF files to exercise format-independent behavior. [PR #1833](https://github.com/catalystneuro/neuroconv/pull/1833)
...
* Added the dict-based metadata format to `IntanRecordingInterface` via `get_metadata(use_new_metadata_format=True)`: the Intan device (manufacturer "Intan", RHD/RHS description) is recorded in the top-level `metadata["Devices"]` registry and referenced by every `metadata["Ecephys"]["ElectrodeGroups"]` entry through `device_metadata_key`, with the amplifier `ElectricalSeries` keyed by `metadata_key`. The default (`get_metadata()`) still returns the old list-based format for backward compatibility. [PR #1815](https://github.com/catalystneuro/neuroconv/pull/1815)
...
* Added support for durative events to `DoricEventsInterface`, for the modern `.doric` HDF5 layout (root group `DataAcquisition`). [PR #1821](https://github.com/catalystneuro/neuroconv/pull/1821)
* `DoricEventsInterface` now also reads the legacy "EPConsole" `.doric` HDF5 layout (`Traces/<console>/<stream>/<stream>` with `DI--O-*` digital lines), auto-detected alongside the modern `DataAcquisition` layout. [PR #1823](https://github.com/catalystneuro/neuroconv/pull/1823)
...
* Added `BaseEventsInterface`, a shared base for events interfaces that writes native `pynwb` 4.0 `EventsTable` objects into `nwbfile.events`. [PR #1774](https://github.com/catalystneuro/neuroconv/pull/1774) [PR #1787](https://github.com/catalystneuro/neuroconv/pull/1787) [PR #1798](https://github.com/catalystneuro/neuroconv/pull/1798)
...
* Added `VameInterface` for converting VAME behavioral segmentation data to NWB. It writes the faithful `ndx-vame` series (per-frame motif labels as `MotifSeries`, optional latent-space embeddings as `LatentSpaceSeries`, optional community labels as `CommunitySeries`, and the VAME project config serialized as JSON) and, for each motif run, a curated `ndx-ethogram` product (the `MotifSeries` run-length-encoded into an `EthogramBouts` timeline with an `Ethogram` catalogue, one row per motif id with the modal community recorded in `category`); the bouts link back to the producing `MotifSeries` (`source`), the upstream `PoseEstimation` (`source_pose`), and, when a project sets `video_metadata_key`, the video `ImageSeries` (`source_video`). A `data_to_write` conversion option (`"algorithm_output"`, `"ethogram"`, or `"both"`, default `"both"`) selects which outputs are written; with `"ethogram"` the faithful `MotifSeries` is absent and each bouts table's `source` back-link to it is dropped. Requires the `ndx-vame` and `ndx-ethogram` extensions. [PR #1737](https://github.com/catalystneuro/neuroconv/pull/1737), [PR #1770](https://github.com/catalystneuro/neuroconv/pull/1770), [PR #1792](https://github.com/catalystneuro/neuroconv/pull/1792), [PR #1803](https://github.com/catalystneuro/neuroconv/pull/1803)
...
* Added `AxonIntracellularConverter`, which combines several `AxonIntracellularInterface` instances (dual-patch channels in one file, or one cell across several protocol files) into a single NWB icephys table. It aligns multi-file recordings on one timeline by their header start times (`rec_datetime`, ABF version 2 only), disambiguates run identities so files that share a Clampex-assigned name (e.g. `0000.abf`) do not collide, and builds the upper icephys hierarchy tables (`SimultaneousRecordings`, `SequentialRecordings`, `Repetitions`, `ExperimentalConditions`) from the per-sweep recordings rows via a format-agnostic aggregator in `tools.icephys`. Optional `repetition` and `condition` labels on the interface group runs into the repetitions and experimental-conditions levels. It also writes a `sweeps` `TimeIntervals` table holding the start and stop time of every sweep, so tools that read NWB intervals (pynapple, for instance) surface the sweeps with no icephys-specific code. [PR #1761](https://github.com/catalystneuro/neuroconv/pull/1761) [PR #1833](https://github.com/catalystneuro/neuroconv/pull/1833)
...
* Added `IntanConverter` for one-call conversion of multi-stream Intan recordings. Parses the `.rhd`/`.rhs` file header via neo, auto-discovers which streams are present, and routes each to `IntanRecordingInterface`, `IntanAnalogInterface`, or `IntanStimInterface` as appropriate. Forwards `saved_files_are_split` to every sub-interface so rotated sessions can be converted in one call as well. [PR #1739](https://github.com/catalystneuro/neuroconv/pull/1739)
...
* Added dict-based metadata pipeline for ecephys in `spikeinterface.py`, supporting the new top-level `Devices` format and a `metadata_key` kwarg on `add_recording_to_nwbfile`. The shape is detected automatically by `_is_dict_based_metadata` and routed by the dispatcher in `add_recording_metadata_to_nwbfile`: dict-based metadata goes through `_add_electrode_groups_to_nwbfile` and `_get_ecephys_metadata_placeholders`; old list-based metadata goes through `_add_electrode_groups_to_nwbfile_old_list_format` (renamed from the previous `_add_electrode_groups_to_nwbfile`). Recording interfaces still emit the old format; their migration is a follow-up. [PR #1743](https://github.com/catalystneuro/neuroconv/pull/1743)
* Added `metadata_key` and a `use_new_metadata_format` parameter to `DeepLabCutInterface.get_metadata()`/`get_metadata_schema()` for the dict-based metadata pipeline: top-level `metadata["Devices"]` and `metadata["Behavior"]["Pose"]` holding `Skeletons` and `PoseEstimations` registries linked via `device_metadata_key`/`skeleton_metadata_key`. `add_to_nwbfile` writes either shape (auto-detected from the metadata), so default behavior is unchanged. The `pose_estimation_metadata_key` argument is deprecated in favor of `metadata_key` and will be removed on or after December 2026. [PR #1750](https://github.com/catalystneuro/neuroconv/pull/1750)
...
## Improvements
* Documented how channel metadata from an external source — a BIDS `channels.tsv`, a lab spreadsheet — reaches the NWB electrodes table, in a new user guide page. Any property set on the recording extractor becomes a column, which was previously discoverable only from the `add_electrodes_to_nwbfile` docstring; the page also covers the property names NeuroConv maps onto the NWB standard (`brain_area` becomes `location`, and so on), that `metadata['Ecephys']['Electrodes']` supplies column descriptions rather than values, and why a unit disagreement has to be corrected through the channel gains instead.
* The TDT events gallery example and `TestTDTFiberPhotometryInterfaceSingleSeries` now read data that exists in gin instead of data that only exists on the CI S3 mirror. The gallery doctest reads `ephy_testing_data/tdt/epocs_with_offsets_1` (24 KB, three durative epoc stores plus a 1 Hz `Tick` clock) instead of the un-stubbed 125 MB `Photo_63_207-181030-103332` photometry tank, and the photometry test reads `Photo_249_391-200721-120136_stubbed` (756 KB, already read by the sibling test class in the same file) instead of `Photometry-161823_stubbed`. The gallery prose, which still described the pre-migration `ndx_events.Events` output, now says the epoc is written as a `pynwb.event.EventsTable` into `nwbfile.events`. [PR #1834](https://github.com/catalystneuro/neuroconv/pull/1834)
* Ecephys, ophys and the video interfaces now write their devices through the top-level `metadata["Devices"]` registry instead of pre-building the entry themselves, and a nested sub-object written inline as metadata (an optical fiber's `fiber_insertion`, an imaging plane's `optical_channel`) is now built from the parent's own constructor spec rather than by hand at each call site. Metadata shapes and written files are unchanged. [PR #1829](https://github.com/catalystneuro/neuroconv/pull/1829)
* `BaseEventsInterface` now writes an event type with zero occurrences as a zero-row `EventsTable` instead of forbidding it. A source that enumerates its event types from a header rather than discovering them from the data (an Intan digital word) can now write an enabled-but-idle line faithfully, instead of dropping it or erroring; whether an empty table is acceptable for archival is left to the NWB Inspector. [PR #1813](https://github.com/catalystneuro/neuroconv/pull/1813)
* On-data test path resolution now honors a `NEUROCONV_TEST_DATA_PATH` environment variable (`tests/test_on_data/setup_paths.py`). Set it once in the shell and every git worktree inherits it, instead of editing a per-tree, gitignored `gin_test_config.json` that each new worktree recreates from the DandiHub default. Fully backward compatible: if the variable is unset, the existing `gin_test_config.json` flow is unchanged. [PR #1811](https://github.com/catalystneuro/neuroconv/pull/1811)
* `BaseFiberPhotometryInterface.get_metadata()` no longer emits a placeholder device/indicator/table scaffold (`"PLACEHOLDER"` strings and `NaN` wavelengths); it now returns only the response-series entry, so an interface writes a bare `FiberPhotometryResponseSeries` unless the user supplies the full provenance chain. [PR #1800](https://github.com/catalystneuro/neuroconv/pull/1800)
* Removed a duplicated definition of `FiberPhotometryInterfaceTestMixin` in `data_interface_mixins.py`. The class was declared twice, so only the second definition was ever in effect; the dead first copy differed from the live one and was the one a reader hit first. No runtime behavior changes. [PR #1796](https://github.com/catalystneuro/neuroconv/pull/1796)
